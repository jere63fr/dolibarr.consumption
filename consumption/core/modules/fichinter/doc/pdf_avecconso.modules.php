 ...
 */
class pdf_avecconso extends ModelePDFFicheinter
{
	 /**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = 'avecconso';
		$this->description = "Modele avec liste des consommations";
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		$outputlangs->loadLangs(array("main", "interventions", "dict", "companies","consumption@consumption"));
		...
		if ($conf->ficheinter->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				//Loop on each consumption
				$conso=new Consumption($this->db);
				$listconso=$conso->fetchconso($object);
				$a=0;
				foreach($listconso as $line){
					if($a==0){
						$txt='<strong>'.$outputlangs->transnoentities("Consumptions").' : </strong><br>';
					}
					$a++;
					$curY = $nexY;
					$desc=$line->product_ref." - ".$line->produit."   " .$outputlangs->transnoentities("Qty")." : ".$line->qty*-1;
					$txt=dol_htmlentitiesbr(dol_concatdesc($txt, $desc), 1, $outputlangs->charset_output);
							$pdf->startTransaction();
					$pdf->writeHTMLCell(0, 0, $curX, $curY + 1, $txt, 0, 1, 0);
					$pageposafter=$pdf->getPage();
					if ($pageposafter > $pageposbefore)	// There is a pagebreak
					{
						$pdf->rollbackTransaction(true);
						...
						//print $pageposafter.'-'.$pageposbefore;exit;
						$pdf->setPageOrientation('', 1, $heightforfooter);	// The only function to edit the bottom margin of current page to set it.
						$pdf->writeHTMLCell(0, 0, $curX, $curY, $txt, 0, 1, 0);
						$pageposafter=$pdf->getPage();
						$posyafter=$pdf->GetY();
						//var_dump($posyafter); var_dump(($this->page_hauteur - ($heightforfooter+$heightforfreetext+$heightforinfotot))); exit;
						if ($posyafter > ($this->page_hauteur - ($heightforfooter+$heightforfreetext+$heightforinfotot)))	// There is no space left for total+free text
